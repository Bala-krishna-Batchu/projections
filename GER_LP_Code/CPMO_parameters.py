...
# GCP Params
# project input data are stored in ANBC env (managed by DE)
BQ_INPUT_PROJECT_ID = "anbc-prod"
BQ_INPUT_DATASET_ENT_CNFV_PROD = "fdl_ent_cnfv_prod"
BQ_INPUT_DATASET_ENT_ENRV_PROD = "fdl_gdp_ae_ent_enrv_prod"

if CLIENT_LOB == 'AETNA':
    BQ_INPUT_DATASET_DS_PRO_LP = "fdl_gdp_ae_ent_enrv_prod"
    BQ_INPUT_DATASET_SANDBOX = "fdl_gdp_ae_ent_enrv_prod"
else:
    BQ_INPUT_DATASET_DS_PRO_LP = "fdl_gdp_ae_ds_pro_lp_share_ent_prod"
    BQ_INPUT_DATASET_SANDBOX = "ds_sandbox"
...
if CLIENT_LOB == 'AETNA':
    AETNA_DATASET_OUTPUT_SUFFIX = ""
    AETNA_TABLE_ID_PREFIX = "gms_aetna_"
else:
    AETNA_DATASET_OUTPUT_SUFFIX = ""
    AETNA_TABLE_ID_PREFIX = ""
...
# Audit Trail
AT_RUN_ID = "" # audit trail ID

#Batch Run ID
BATCH_ID = ""

# If set to True, the client YTD projections will be directly read from BQ tables.
...
PROGRAM_INPUT_PATH = "gs://pbm-mac-lp-prod-ai-bucket"
PROGRAM_OUTPUT_PATH = "/home/jupyter"
...
GUARANTEE_CATEGORY = "Pure Vanilla"
...
DATA_START_DAY = "False"
LAST_DATA = dt.datetime.strptime('1/31/2021', '%m/%d/%Y')

# The date of when the price changes will go live. To be set in close coordination with the business side (IA Team)
GO_LIVE = dt.datetime.strptime('03/01/2021', '%m/%d/%Y')
...
BIG_CAPPED_PHARMACY_LIST = {'GNRC': ['CVS', 'RAD', 'WAG', 'KRG', 'WMT'], 'BRND': ['CVS', 'RAD', 'WAG', 'KRG', 'WMT']}
SMALL_CAPPED_PHARMACY_LIST = {'GNRC': ['AMZ', 'HYV', 'KIN', 'ABS', 'PBX', 'AHD', 'GIE', 'MJR', 'GEN', 'TPS'], 'BRND': ['ABS', 'AHD', 'PBX', 'MJR', 'WGS', 'SMC', 'CST', 'KIN', 'GIE', 'HYV', 'TPM', 'SMR', 'ARX', 'GEN', 'DDM', 'PMA', 'GUA', 'BRI', 'MGM', 'AMZ', 'TPS']}
NON_CAPPED_PHARMACY_LIST = {'GNRC': ['TPM', 'BRI', 'PMA', 'ARX', 'RUR', 'DDM', 'GUA', 'CHD', 'CST', 'SMC', 'SMR', 'WGS', 'NONPREF_OTH', 'MGM'], 'BRND': ['RUR', 'CHD', 'NONPREF_OTH']}
COGS_PHARMACY_LIST = {'GNRC': [], 'BRND': []}
PSAO_LIST = {'GNRC': ['ELE', 'EPC', 'HMA', 'CAR'], 'BRND': ['ELE', 'EPC', 'HMA', 'CAR']}
...
MEDD_MCHOICE_TARGET_RATE = 0.8905
...
CONFLICT_GPI_AS_TIERS_BOUNDS = 1

# The lower bound for the conflict GPI price tier
CONFLICT_GPI_LOW_BOUND = 0.8
...
PARITY_PRICE_DIFFERENCE_COLLAR_HIGH = 1.25 
PARITY_PRICE_DIFFERENCE_COLLAR_LOW = 0.75 
...
#================================================================
# Parameter to allow for UCL pricing.
UCL_CLIENT = False
...
PRICE_OVERRIDE_FILE = 'Price_Overrides_{}.csv'.format(DATA_ID)  
...
UNC_ADJUST = False
UNC_ADJUSTMENT = 'UC_Adjustment_{}.csv'.format(DATA_ID)
...
UNC_PHARMACY = False
if CLIENT_TYPE == 'MEDD':
    UNC_PHARMACY_CHAIN_GROUPS = ['WMT']
elif (CLIENT_TYPE == 'COMMERCIAL' or CLIENT_TYPE == 'MEDICAID'):
    UNC_PHARMACY_CHAIN_GROUPS = ['WAG', 'WMT']
...
#Sets the ceilings for Intercept_High with respect to GoodRx prices
INTERCEPT_CEILING = 3
...
GOODRX_OPT = False
RAW_GOODRX = ''
GOODRX_FILE = 'GoodRx_Limits_{}.csv'.format(DATA_ID)
COSTPLUS_FILE = 'Costplus_Limits_{}.csv'.format(DATA_ID)
GOODRX_MATCH_METHOD = 'forward'
GOODRX_QTY_METHOD = 'median'
...
BRAND_SURPLUS_FILE = 'brand_surplus_{}.csv'.format(DATA_ID)
...
BRAND_SURPLUS_REPORT = 'brand_surplus_report_{}.csv'.format(DATA_ID)
...
# Currently only implemented for Pacificsource and it's associated customer_ids. Set SPECIALTY_OFFSET = False if FULL_YEAR = True. 
SPECIALTY_OFFSET = True
# Contains customer IDs and projected specialty surplus for the whole contract period.
SPECIALTY_SURPLUS_DATA = 'spclty_surplus_20240213.csv'
...
if CROSS_CONTRACT_PROJ == True:
    CCP_SUFFIX = '_CCP' + '_{}_{}'.format(str(dt.date.today().year - 1), str(dt.date.today().year))
else:
    CCP_SUFFIX = "''"
...
# on current contract data, or set to 'None' to not use the ranksings list. 'None' option only works with LEAKAGE_OPT, all others 
...
LEAKAGE_COPAY_SIMPLE_BREAKPOINT = 'AVG_COPAY_UNIT'
...
# Setting this incorrectly will cause a key error.
LEAKAGE_COPAY_COMPLEX_BREAKPOINT = 'AVG_COPAY_UNIT'
...
# Setting this incorrectly will cause a key error.
LEAKAGE_COINSURANCE_SIMPLE = 'AVG_COINS'
...
# Setting this incorrectly will cause a key error.
LEAKAGE_COINSURANCE_COMPLEX = 'AVG_COINS'
...
FILE_INPUT_PATH = os.path.join(PROGRAM_INPUT_PATH, 'Input/')
FILE_OUTPUT_PATH = os.path.join(PROGRAM_OUTPUT_PATH, 'Output/')
FILE_DYNAMIC_INPUT_PATH = os.path.join(PROGRAM_OUTPUT_PATH, 'Dynamic_Input/')
FILE_LOG_PATH = os.path.join(PROGRAM_OUTPUT_PATH, 'Logs/')
FILE_LP_PATH = os.path.join(PROGRAM_OUTPUT_PATH, 'LP/')
FILE_REPORT_PATH = os.path.join(PROGRAM_OUTPUT_PATH, 'Report/')
...
CLIENT_NAME_MAPPING_FILE = 'Client_Name_ID_Mapping_{}.csv'.format(DATA_ID)

# List of MAC prices for selected client(s)
MAC_LIST_FILE = 'Mac_List_{}.csv'.format(DATA_ID)

# List of the minimum prices (MAC 1026 floor prices) for each GPI-NDC
RAW_MAC1026_FILE = 'raw_MAC1026_{}.csv'.format(DATA_ID)
MAC1026_FILE = 'MAC1026_{}.csv'.format(DATA_ID)

# List of Beg_Q_Price and Beg_M_Price for WTW/AON clients
DRUG_MAC_HIST_FILE = 'Drug_MAC_History_{}.csv'.format(DATA_ID)

# List of the NADAC AND WAC prices for each GPI-NDC
RAW_NADAC_WAC_FILE = 'raw_NADAC_WAC_{}.csv'.format(DATA_ID)
NADAC_WAC_FILE = 'NADAC_WAC_{}.csv'.format(DATA_ID)

# BREAKOUT MAPPING FILE FOR THE CLIENT
RAW_BREAKOUT_MAPPING_FILE = 'RAW_BREAKOUT_MAPPING_FILE_{}.csv'.format(DATA_ID)
BREAKOUT_MAPPING_FILE = 'BREAKOUT_MAPPING_FILE_{}.csv'.format(DATA_ID)

# The client(s) guarantees for the different breakouts
CLIENT_GUARANTEE_FILE = 'Client_Guarantees_{}.csv'.format(DATA_ID)

# Client guarantees from before a market check
CLIENT_GUARANTEE_PREMC_FILE = 'Client_Guarantees_Premc_{}.csv'.format(DATA_ID)

# List of the commerical pharmacy guarantees for all commercial clients
COMMERCIAL_PHARM_GUARANTEES_FILE = 'Pharm_Guarantees_commercial_{}.csv'.format(DATA_ID)

# List of the medicaid pharmacy guarantees for all medicaid clients
MEDICAID_PHARM_GUARANTEES_FILE = 'Pharm_Guarantees_medicaid_{}.csv'.format(DATA_ID)

# Pharmacy guarantees with client information after pre-processing
PHARM_GUARANTEE_FILE = 'Pharm_Guarantees_{}.csv'.format(DATA_ID)

# The VCML information for a specific client(s)
# read into pre-processing
VCML_REFERENCE_FILE = 'VCML_Reference_{}.csv'.format(DATA_ID)
# after pre-processing
MAC_MAPPING_FILE = 'Mac_Mapping_{}.csv'.format(DATA_ID)

#Customized new year region map mapping file for MEDD new year pricing
CUSTOM_MAC_MAPPING_FILE = 'Custom_Mac_Mapping_2024.csv'

# Pharmacy claims at the month level (aggregated)
# read into pre-processing
PHARMACY_CLAIM_FILE = 'Pharmacy_Raw_Claims_{}.csv'.format(DATA_ID)
# Year to date pharmacy performance by breakout, measurement, and chain group
# after pre-processing
PHARMACY_YTD = 'YTD_Pharmacy_Performance_{}.csv'.format(DATA_ID)

# Client and pharmacy claims table claim level
CLIENT_PHARMACY_CLAIM_FILE = ''

# Non-mac rates per VCML
NON_MAC_RATE_FILE = 'Non_Mac_Rate_{}.csv'.format(DATA_ID)

# The generic launch file before being modify for proper code use (format)
# read into pre-processing
RAW_GEN_LAUNCH_FILE = 'gen_launch_{}.csv'.format(DATA_ID)
# The generic launch file after being modified for proper code use (format)
# after pre-processing
GENERIC_LAUNCH_FILE = 'generic_launch_{}.csv'.format(DATA_ID)
...
RAW_MAC_PRICE_OVERRIDES = 'RAW_GER_OPT_MAC_Price_Overrides_{}.csv'.format(DATA_ID)
# after pre-processing
MAC_PRICE_OVERRIDE_FILE = 'MAC_Price_Overrides_{}.csv'.format(DATA_ID)
# Parameter to treat MAC PRICE OVERRIDE GPIs as Overrides (True) or Exclusions(False)
MAC_PRICE_OVERRIDE = True
# WMT UNC OVERRIDE FILE NAME
WMT_UNC_PRICE_OVERRIDE_FILE = 'WMT_UNC_Price_Overrides_{}.csv'.format(DATA_ID)

# Information about a package size for a GPI-NDC combination
PACKAGE_SIZE_FILE = 'package_size_to_ndc_{}.csv'.format(DATA_ID)

# List of backout GPIs
BACKOUT_GEN = 'Gen_Launch_Backout_{}.csv'.format(DATA_ID)
...
MEASUREMENT_MAPPING = 'Measurement_mapping_{}.csv'.format(DATA_ID)

# The mapping of VCMLs to guarantee equal prices on a VCML level
MAC_CONSTRAINT_FILE = 'Mac_Constraints_{}.csv'.format(DATA_ID)

# Client(s) claims at the GPI level
DAILY_TOTALS_FILE = 'Daily_Total_{}.csv'.format(DATA_ID)

# Projections data at the DAY level
EOY_PROJ_FILE = 'EOY_PROJ_DAY_{}.csv'.format(DATA_ID)

# Scale factor values for the pharmacy performance approximation
PHARMACY_APPROXIMATIONS = 'Pharmacy_approx_coef_{}.csv'.format(DATA_ID)
PHARMACY_APPROXIMATIONS_XY_DATA = 'Pharmacy_approx_XYdata_{}.csv'.format(DATA_ID)

# List of GPIs to be excluded from the algorithm because they are specialty drugs
RAW_SPECIALTY_EXCLUSION_FILE = 'Raw_GPI_change_exclusion_NDC_{}.csv'.format(DATA_ID)

# List of GPIs to be excluded from the algorithm because of MONY changes
RAW_MONY_EXCLUSION_FILE = ''

SPECIALTY_EXCLUSION_FILE = 'GPI_change_exclusion_NDC_{}.csv'.format(DATA_ID)

#List of GPIs to be excluded from the algorithm because they cause an infeasibility
INFEASIBLE_EXCLUSION_FILE = 'infeasible_exclusion_gpis_{}.csv'.format(CUSTOMER_ID[0])

#List of GPIs to be excluded from the algorithm because they have conflicting R30/R90/M30 prices
CONFLICT_GPI_LIST_FILE = 'conflict_exclusion_gpis_{}_{}_{}.csv'.format(CUSTOMER_ID[0], dt.date.today().strftime('%B'), RUN_TYPE_TABLEAU)
#Conflicting GPIs generated this round
CONFLICT_GPI_LIST_FILE_THIS_RUN = 'conflict_exclusion_gpis_this_run_{}_{}_{}.csv'.format(CUSTOMER_ID[0], dt.date.today().strftime('%B'),RUN_TYPE_TABLEAU)

# The Other client pharmacy performance is only used for Med-D clients
OC_PHARM_PERF_FILE = 'all_other_medd_client_perf_{}.csv'.format(DATA_ID)

# History of GPI-NDC AWP prices
AWP_FILE = 'AWP_HISTORY_TABLE_{}.csv'.format(DATA_ID)

# GPI Classes file
GPI_CLASSES = 'GPI & CLASSES.xlsx'
...
PREFERRED_PHARM_FILE = 'Pref_Pharm_List_{}.csv'.format(DATA_ID)

# UNC Daily totals and Percentiles files
UNC_DAILY_TOTALS_FILE = 'UNC_Daily_Total_{}.csv'.format(DATA_ID)
UNC_GPI_PERCENTILES_FILE = 'UNC_GPI_percentiles_constrained_{}.csv'.format(DATA_ID)  # UCAMT percentiles for all claims at GPI level
UNC_NDC_PERCENTILES_FILE = 'UNC_NDC_percentiles_constrained_{}.csv'.format(DATA_ID)  # UCAMT percentiles for all claims at GPI-NDC level

# Daily Totals Report
FILE_DAILY_TOTALS_REPORT = 'daily_totals_{}.sql'.format(DATA_ID)

# Value Report due to INTERCEPTOR_OPT = True
INTERCEPTOR_VALUE_REPORT = 'Interceptor_value_{}.CSV'.format(DATA_ID)
# The leakage parametrizes the claim movlume that will be miss labeled
INTERCEPTOR_LEAKAGE = 0.05
# Determines the lenght of the timeperiod to determine if a drug is zbd or not
INTERCEPTOR_ZBD_TIME_LAG = 1

#U&C EBIT report
UNC_EBIT_REPORT = 'unc_ebit_report_{}.csv'.format(DATA_ID)

# Current contract data file
CURRENT_CONTRACT_DATA_FILE = 'current_contract_data_{}.csv'.format(DATA_ID)

# Aggregated plan design files
PLAN_DESIGN_FILE_GPI = 'plan_design_file_gpi_{}.csv'.format(DATA_ID)
PLAN_DESIGN_FILE_NDC = 'plan_design_file_ndc_{}.csv'.format(DATA_ID)

# Specialty Surplus file
SPECIALTY_SURPLUS_FILE = 'spclty_surplus_file_{}.csv'.format(DATA_ID)

# Market check data files
PRE_MC_DATA_FILE = 'pre_mc_data_{}.csv'.format(DATA_ID)
POST_MC_DATA_FILE = 'post_mc_data_{}.csv'.format(DATA_ID)
...
PHARMACY_OUTPUT = 'LP_Algorithm_Pharmacy_Output_Month_{}.csv'.format(DATA_ID)

PRICE_CHECK_OUTPUT = 'Price_Check_Output_{}.csv'.format(DATA_ID)

TOTAL_OUTPUT = 'Total_Output_{}.csv'.format(DATA_ID)

PRE_EXISTING_PERFORMANCE_OUTPUT = 'Pre_existing_Performance_{}_{}.csv'.format(str(GO_LIVE.month), DATA_ID)

MODEL_PERFORMANCE_OUTPUT = 'Model_Performance_{}_{}.csv'.format(str(GO_LIVE.month), DATA_ID)

MODEL_02_PERFORMANCE_OUTPUT = 'Model_02_Performance_{}_{}.csv'.format(str(GO_LIVE.month), DATA_ID)

LAMBDA_PERFORMANCE_OUTPUT = 'Lambda_Performance_{}_{}.csv'.format(str(GO_LIVE.month), DATA_ID)

PRICE_CHANGE_FILE = '{}_{}_{}.txt'.format(CLIENT_NAME_TABLEAU, CLIENT_TYPE, GO_LIVE.strftime('%m%d%y'))

DIAGNOSTIC_REPORT = 'Diagnostic_Report_{}.csv'.format(DATA_ID)

CONTRACT_DATE_FILE = 'Contract_Dates_{}.csv'.format(DATA_ID)

PRICE_CHANGES_OUTPUT_FILE = '{}_Price_Changes_{}_{}.xlsx'.format(CUSTOMER_ID[0], TIMESTAMP, str(GO_LIVE.month))
...
TABLE_RAW_CLAIMS = GER_PREFIX + USER + '_' + 'raw_clm3'
if CLIENT_TYPE == 'MEDD':
    TABLE_MEASUREMENT_MAPPING = GER_PREFIX + DATA_START_DAY[:4] +  CLIENT_NAME_TABLEAU + '_' + 'msrmnt_map3'
elif (CLIENT_TYPE == 'COMMERCIAL' or CLIENT_TYPE == 'MEDICAID'):
    TABLE_MEASUREMENT_MAPPING = GER_PREFIX + USER + '_' +'msrmnt_map3'
TABLE_MAPPED_CLAIMS = GER_PREFIX + USER + '_' + 'all_mac_and_uc3'
TABLE_MEDIAN_AND_25TH = GER_PREFIX + USER + '_'+ 'percentile3'
TABLE_AUTOMATION_DAILY_TOTALS = GER_PREFIX + USER + '_'+ 'daily_totals3'

TABLE_UNC_AUTOMATION_DAILY_TOTALS = GER_UNC_PREFIX+ USER + '_' + 'daily_total'
TABLE_UNC_RAW_CLAIMS = GER_UNC_PREFIX+ USER + '_' + 'raw_clm3'
TABLE_UNC_MAC_1026 = GER_UNC_PREFIX+ USER + '_' +'mac_1026'
TABLE_UNC_ALL_MAC_AND_UC_CLAIMS = GER_UNC_PREFIX + USER + '_' + 'claims3'
TABLE_UNC_ALL_MAC_AND_UC_CLAIMS_MAC = GER_UNC_PREFIX + USER + '_' + 'MAC3'

# tables used to create the percentiles for the U&C algorithm
TABLE_UNC_FILTERED_MAC_AND_UNC_PERCENTILES_NDC = GER_UNC_PREFIX + USER + '_' + 'NDC_prcntl'
TABLE_UNC_PERCENTILES_NDC_CONSTRAINTS_MAC_NDC = GER_UNC_PREFIX + USER + '_' + 'NDC_cnstrn'
TABLE_UNC_FILTERED_MAC_AND_UNC_PERCENTILES_GPI = GER_UNC_PREFIX + USER + '_' + 'GPI_prcntl'
TABLE_UNC_PERCENTILES_GPI_CONSTRAINTS_MAC_GPI = GER_UNC_PREFIX + USER + '_' + 'GPI_cnstrn'
...
TIERED_PRICE_CLIENT = CUSTOMER_ID
...
MAIL_TARGET_BUFFER = -0.0

# Performance buffer on retail, number derived from the business
RETAIL_TARGET_BUFFER = -0.0

#Price Bounds for FULL_YEAR: Level 1
FULL_YEAR_LV_1_UPPER_BOUND = [8, 25, 50, 100, 999999]
FULL_YEAR_LV_1_MAX_PERCENT_INCREASE = [20000, 0.6, 0.35, 0.25, 0.15]
FULL_YEAR_LV_1_MAX_DOLLAR_INCREASE = [8, 999999, 999999, 999999, 999999]
...
FULL_YEAR_LV_2_UPPER_BOUND = [8, 25, 50, 100, 999999]
FULL_YEAR_LV_2_MAX_PERCENT_INCREASE = [20000, 1, 0.75, 0.35, 0.25]
FULL_YEAR_LV_2_MAX_DOLLAR_INCREASE = [10, 999999, 999999, 999999, 999999]
...
FULL_YEAR_LV_3_UPPER_BOUND = [8, 25, 50, 100, 999999]
FULL_YEAR_LV_3_MAX_PERCENT_INCREASE = [20000, 1.5, 1.0, 0.5, 0.35]
FULL_YEAR_LV_3_MAX_DOLLAR_INCREASE = [12, 999999, 999999, 999999, 999999]
...
FULL_YEAR_LV_4_UPPER_BOUND = [3, 6, 999999]
FULL_YEAR_LV_4_MAX_PERCENT_INCREASE = [20000, 20000, 3]
FULL_YEAR_LV_4_MAX_DOLLAR_INCREASE = [20, 30, 999999]
...
LAG_YTD_OVERRIDE = False
YTD_OVERRIDE = False
LAG_YTD_Override_File = ''
...
ALLOW_INTERCEPT_LIMIT = False

# They modify the weight of the Lambda variables on the objective function.
OVER_REIMB_GAMMA = 0.1
if CLIENT_TYPE == 'MEDD':
    OVER_REIMB_CHAINS = {'GNRC': ['CVS','WAG','WMT'],
                         'BRND': ['CVS','WAG','WMT']}
elif (CLIENT_TYPE == 'COMMERCIAL' or CLIENT_TYPE == 'MEDICAID'):
    OVER_REIMB_CHAINS = {'GNRC': ['CVS', 'WAG', 'KRG', 'WMT'],
                         'BRND': ['CVS', 'WAG', 'KRG', 'WMT']}
else:
    raise Exception(f'CLIENT_TYPE {CLIENT_TYPE} not supported in CPMO_parameters yet.')
...
BRAND_GENERIC_UNRESTRICTED = False
GENERIC_UNRESTRICTED_CAP = 1.0
...
UPPER_BOUND = [5, 10, 25, 50, 100, 999999]
MAX_PERCENT_INCREASE = [20, 1, 0.4, 0.3, 0.2, 0.1]
MAX_DOLLAR_INCREASE = [5, 10, 10, 15, 20, 20000]
PRICE_BOUNDS = {
...
MBR_GOODRX_COMPETITIVE_MULTIPLIER_SAME = [None,1.1,1.1]
...
MBR_GOODRX_COMPETITIVE_MULTIPLIER_CHAIN = [None,1.2,1.2]
...
#MBR_PRICING_TIER_CHAIN = [25000000, 50000000] 
...
PHARM_GUARANTEE_COMM_NY= 'Pharmacy_Rate_Commercial_Next_Year_2025.csv'
PHARM_GUARANTEE_MEDD_NY= 'Pharmacy_Rate_MEDD_Next_Year_2025.csv'
PHARM_GUARANTEE_MEDICAID_NY= 'Pharmacy_Rate_Medicaid_Next_Year_2025.csv'
CLIENT_GUARANTEE_MEDD_NY= 'Client_Rate_MEDD_Next_Year_2025.csv'
CUSTOMER_ID_MAPPING_MEDD_NY= 'CUSTOMER_ID_MAPPING_MEDD_NY_2025.csv'
VCML_CROSSWALK_MEDD_2024 = 'VCML_CROSSWALK_MEDD_2024.csv'
#=====================================================================================================

# Remove WTW Restriction
REMOVE_WTW_RESTRICTION= False
...
PHARM_TARGET_RATE_RATIO = 0.95

# Parameter to ignore the mentioned VCMLS in QA.py and qa_checks.py 
IGNORED_VCMLS = ['10','12','13','SX','SX1','S3','S9', 'E23', 'N23','E92', '92', '24', 'E77', 'P77', '77', 'E78', '78','88','A1','A2','A3','A4','A5','L1','L2','L3','L4','L5','LD','M78','69','E69','70','E70','57','86','E86']
...
XA_XR_LIST = ['4667','3211','3645','4134','4139','4443', '4450', '5746','265C', '4683', '4047', '156C','295C', '176C' ]

# Parameter to set pref_pharm = 'None'. This is due to the preferred pharmacy logic in the combined measurement mapping table being fault causing the LP fail QA checks about pref_pharms.
PREF_PHARM_BYPASS=False
...
# Default is TRUECOST_CLIENT = False, where it will use current_mac in LP soft constraint
    
if TRUECOST_CLIENT:
    APPLY_VCML_PREFIX = 'TRU'
elif UCL_CLIENT:
    APPLY_VCML_PREFIX = 'UCL'
else:
    APPLY_VCML_PREFIX = 'MAC'
...
TRUECOST_UPPER_MULTIPLIER_GNRC = 1.249
TRUECOST_UPPER_MULTIPLIER_BRND = 1.149

#=================================================================
# List of clients that will get uniform pricing for all VCMLs. These clients are not adjudicating as expected based on the VCML setup.
UNIFORM_MAC_PRICING = True
...
#    the constraints will be ratio * (1-SM_THERA_COLLAR_LOW) * ratio <= new_drug1/new_drug2 <= (1+SM_THERA_COLLAR_HIGH) * ratio
