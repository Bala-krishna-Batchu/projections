...
ndc_awp_table_custom = """
SELECT
    GPI,
    NDC,
    BG_FLAG,
    MAX(DRUG_PRICE_AT) AS AWP_LIST_NDC
FROM `{_project_id}.{_dataset_id}.{_table_id}`
WHERE CURRENT_DATE() BETWEEN EFF_DATE AND EXP_DATE0
    AND CHRG_BASIS_QLFR = 'AWP'
GROUP BY 1,2,3
"""

gpi_awp_table_custom = """
SELECT
    GPI,
    BG_FLAG,
    MAX(DRUG_PRICE_AT) AS AWP_LIST_GPI
FROM `{_project_id}.{_dataset_id}.{_table_id}`
WHERE CURRENT_DATE() BETWEEN EFF_DATE AND EXP_DATE0
    AND CHRG_BASIS_QLFR = 'AWP'
GROUP BY 1,2
...
client_guarantees_dummy_TC = """
SELECT * FROM
(SELECT distinct CUSTOMER_ID AS CLIENT, REGION, BREAKOUT, MEASUREMENT, BG_FLAG, CAST (0.0 as FLOAT64) AS Rate, 'Non_Preferred' AS PHARMACY_TYPE  from `{_project}.{_landing_dataset}.{_table_id}`

UNION ALL 

SELECT distinct CUSTOMER_ID AS CLIENT, REGION, BREAKOUT, MEASUREMENT, BG_FLAG, CAST (0.0 as FLOAT64) AS Rate, 'Preferred' AS PHARMACY_TYPE from `{_project}.{_landing_dataset}.{_table_id}`
)
where CLIENT in ({_customer_id})
AND MEASUREMENT NOT LIKE '%S%'
"""

eff_client_guarantees_TC = """
SELECT
CLIENT,
BREAKOUT,
MEASUREMENT, 
BG_FLAG,
'Non_Preferred' AS PHARMACY_TYPE,
CAST (EFF_GER as FLOAT64) AS RATE
"""
...
SELECT
  ...
  guarantee_category AS Guarantee_Category,
  no_of_vcmls AS No_of_Vcmls
FROM {_project}.{_dataset}.{_table_id}
...
SELECT
  ...
  CAST(breakout AS STRING) AS BREAKOUT,
  CAST(region AS STRING) AS Region,
  CAST(BG_FLAG AS STRING) AS BG_FLAG,
  measurement AS MEASUREMENT,
  preferred AS Preferred,
  chain_group AS CHAIN_GROUP,
  ...
  CAST(qty AS float64) AS QTY,
  CAST(awp AS float64) AS AWP,
  CAST(nadac_wac_ytd AS float64) AS NADAC_WAC_YTD,
  CAST(spend AS float64) AS SPEND,
  CAST(member_cost AS float64) AS MEMBER_COST,
  CAST(disp_fee AS float64) AS DISP_FEE,
  CAST(TARGET_DISP_FEE AS float64) AS TARGET_DISP_FEE,
  ...
  CAST(PHARMACY_NADAC AS float64) AS PHARMACY_NADAC,
  CAST(PHARMACY_ACC AS float64) AS PHARMACY_ACC,
  CAST(PHARMACY_TARGET_IC_COST AS float64) AS PHARMACY_TARGET_IC_COST,
  CAST(PHARMACY_SPEND AS float64) AS PHARMACY_SPEND,
  CAST(PHARMACY_MEMBER_COST AS float64) AS PHARMACY_MEMBER_COST,
  CAST(CASE WHEN PHARMACY_TARGET_DISP_FEE IS NULL THEN 0 ELSE PHARMACY_DISP_FEE END AS float64) AS PHARMACY_DISP_FEE,
  CAST(CASE WHEN PHARMACY_TARGET_DISP_FEE IS NULL THEN 0 ELSE PHARMACY_TARGET_DISP_FEE END AS float64) AS PHARM_TARGET_DISP_FEE,
  CAST(CLAIMS_distinct_ZBD AS float64) AS CLAIMS_ZBD,
  CAST(QTY_ZBD AS float64) AS QTY_ZBD,
  CAST(AWP_ZBD AS float64) AS AWP_ZBD,
  CAST(spend_ZBD AS float64) AS SPEND_ZBD,
  CAST(member_cost_ZBD AS float64) AS MEMBER_COST_ZBD,
  CAST(disp_fee_ZBD AS float64) AS DISP_FEE_ZBD,
  CAST(TARGET_DISP_FEE_ZBD AS float64) AS TARGET_DISP_FEE_ZBD,
  CAST(PHARMACY_CLAIMS_ZBD AS float64) AS PHARMACY_CLAIMS_ZBD,
  CAST(PHARMACY_QTY_ZBD AS float64) AS PHARMACY_QTY_ZBD,
  CAST(PHARMACY_AWP_ZBD AS float64) AS PHARMACY_AWP_ZBD,
  CAST(PHARMACY_NADAC_ZBD AS float64) AS PHARMACY_NADAC_ZBD,
  CAST(PHARMACY_ACC_ZBD AS float64) AS PHARMACY_ACC_ZBD,
  CAST(PHARMACY_TARGET_IC_COST_ZBD AS float64) AS PHARMACY_TARGET_IC_COST_ZBD,
  CAST(PHARMACY_SPEND_ZBD AS float64) AS PHARMACY_SPEND_ZBD,
  CAST(PHARMACY_MEMBER_COST_ZBD AS float64) AS PHARMACY_MEMBER_COST_ZBD,
  CAST(CASE WHEN PHARMACY_TARGET_DISP_FEE_ZBD IS NULL THEN 0 ELSE PHARMACY_DISP_FEE_ZBD END AS float64) AS PHARMACY_DISP_FEE_ZBD,
  CAST(CASE WHEN PHARMACY_TARGET_DISP_FEE_ZBD IS NULL THEN 0 ELSE PHARMACY_TARGET_DISP_FEE_ZBD END AS float64) AS PHARMACY_TARGET_DISP_FEE_ZBD,
...
truecost_specialty_exclusion_custom = """
WITH PCD_GPI AS (
  SELECT DISTINCT GPI, speciality_level_ldd
  FROM `anbc-prod.fdl_gdp_ae_ent_enrv_prod.gms_truecost_gpi_14_drug_list`
  WHERE faf_id IN (
      SELECT DISTINCT faf_id
      FROM `anbc-prod.fdl_gdp_ae_ent_enrv_prod.gms_truecost_client_info`
      WHERE CUSTOMER_ID IN ({_customer_id})
        AND '{_contract_eff_date}' BETWEEN contract_start AND contract_end
    )
),
NTM_DRUGS AS (
  SELECT DISTINCT TRIM(DH.gpi_cd) AS GPI
  FROM `anbc-prod.fdl_gdp_ae_ent_enrv_prod.spclt_drug_hist_ds_tmp` SRX
  JOIN `anbc-prod.fdl_gdp_ae_ent_enrv_prod.drug_hist_ds_tmp` DH
    ON DH.PROD_GID = SRX.DRUG_PROD_GID
    AND SRX.DRUG_PROD_GID <> -2
    AND TRIM(DH.SRC_CD) = 'D'
  WHERE CURRENT_DATE('America/New_York') BETWEEN SRX.EFF_DT AND SRX.EXPRN_DT
    AND (SRX.SPCLT_DRUG_IND = '1'
      OR TRIM(SRX.DISTRIBUTION_CD) IN (
        'LIMITED DISTRIBUTION SPECIALTY DRUG WITHOUT ACCESS',
        'LIMITED DISTRIBUTION NON-SPECIALTY DRUG WITHOUT ACCESS',
        'LIMITED DISTRIBUTION SPECIALTY DRUG WITH ACCESS',
        'LIMITED DISTRIBUTION NON-SPECIALTY DRUG WITH ACCESS'
      ))
    AND TRIM(DH.gpi_cd) NOT IN (SELECT GPI FROM PCD_GPI)
)
-- Specialty GPIs from PCD
SELECT DISTINCT GPI, 'G' AS BG_FLAG FROM PCD_GPI
WHERE speciality_level_ldd IS NOT NULL AND speciality_level_ldd != 'N/A'
UNION DISTINCT
-- NTM GPIs not in PCD
SELECT GPI, 'G' AS BG_FLAG FROM NTM_DRUGS
"""
...
#Pulling the MAC LISTs with GPPC for loading prices#
mac_list_ADHOC_custom = """
SELECT
  CAST(mac AS STRING) AS MAC,
  CAST(gpi AS STRING) AS GPI,
  CAST(ndc AS STRING) AS NDC,
  ...
  mac_list AS MAC_LIST,
  TRUE as IS_MAC
FROM `{_project}.{_dataset_id}.{_table_id}` tru
INNER JOIN `anbc-prod.fdl_gdp_ae_ent_enrv_prod.v_gms_ger_opt_base_mac_lists` base_mac
ON tru.MAC = base_mac.mac_list_id 
  AND tru.GPI = base_mac.mac_gpi_cd 
  AND tru.NDC = base_mac.drug_id   
  AND MAC_COST_AMT > 0
  AND curr_ind='Y'
  AND MAC_EXPRN_DT > CURRENT_DATE('America/New_York')
  AND VALID_GPPC = 'Y'
WHERE 
  mac IN (
          SELECT
            DISTINCT vcml_id
          FROM
            `{_project}.{_dataset_id}.{_vcml_ref_table_id}`
          WHERE
...
SELECT
  ...
  CAST(gpi AS STRING) AS GPI,
  CAST(ndc AS STRING) AS NDC,
  generic_price AS GENERIC_PRICE,
  brand_price AS BRAND_PRICE,
  mac_list AS MAC_LIST,
  CASE
    WHEN IS_MAC = 'N' THEN FALSE
    ELSE TRUE 
    END AS IS_MAC
""" 
...
SELECT
  UPPER(pharmacy_group) AS PHARMACY_GROUP,
  UPPER(pharmacy_subgroup) AS PHARMACY_SUBGROUP,
  CAST(target_rate AS FLOAT64) AS TARGET_RATE, 
  PHRM_GRTE_TYPE,
  BG_FLAG
"""

medicaid_pharm_guarantees = f"""
SELECT
  UPPER(pharmacy_group) AS PHARMACY_GROUP,
  UPPER(pharmacy_subgroup) AS PHARMACY_SUBGROUP,
  CAST(target_rate AS FLOAT64) AS TARGET_RATE,
...
SELECT
    ...
    CAST(IS_MAC AS BOOL) AS IS_MAC,
    CAST(IS_SPECIALTY AS BOOL) AS IS_SPECIALTY,
    FULLAWP_ADJ,
    FULLAWP_ADJ_PROJ_LAG,
    FULLAWP_ADJ_PROJ_EOY,
    PRICE_REIMB,
    ...
    TARG_INGCOST_ADJ,
    TARG_INGCOST_ADJ_PROJ_LAG,
    DISP_FEE,
    DISP_FEE_PROJ_LAG,
    DISP_FEE_PROJ_EOY,
    TARGET_DISP_FEE,
    TARGET_DISP_FEE_PROJ_LAG,
    TARGET_DISP_FEE_PROJ_EOY,
    COMPETITIVE_SCORE,
    CLAIMS,
    CLAIM_PRICE_RATIO
"""

pharm_awp_spend_total = f"""
SELECT
    CAST(CLIENT AS STRING) AS CLIENT,
    CAST(BREAKOUT AS STRING) AS BREAKOUT,
    CAST(REGION AS STRING) AS REGION,
...
"""
#Aetna cannot replace original contract effective date with contract effect date 
...
FROM {_project}.{_landing_dataset}.gms_v_gms_summaryprocess  s
...
AND CAST(s.fromdate AS DATE) BETWEEN PARSE_DATE('%m/%d/%Y',t.CONTRACTSTART) AND PARSE_DATE('%m/%d/%Y',t.CONTRACTEND)
AND NOT REGEXP_CONTAINS(UPPER(ClientName),r'SPECIALTY|LTC|LDD|WRAP|HIF|EXC ZBD|BIOSIMILAR|_IHS|_TER')
AND (UPPER(ClientName) NOT LIKE '%ALF%' OR UPPER(ClientName) LIKE '%BALFOUR%' OR UPPER(ClientName) LIKE '%ALFRED NICKLES BAKERY INC%') 
...
SELECT
 ...
 , GPI
 , 'G' AS BG_FLAG
 , ROUND(weighted_mean,4) AS {_vendor}_PRICE
 , ROUND(buffer_high,4) AS {_vendor}_BUFFER_HIGH
 , ROUND(buffer_low,4) AS {_vendor}_BUFFER_LOW
'''

cs_pharm_guarantees_query = """
SELECT DISTINCT
  yr,
  CASE 
    WHEN PHARMACY_GROUP = 'AHOLD'THEN 'AHD'
    WHEN PHARMACY_GROUP ='ALBERTSONS'THEN 'ABS'
    WHEN PHARMACY_GROUP ='CARDINAL'THEN 'CAR'
    WHEN PHARMACY_GROUP ='ELEVATE'THEN 'ELE'
    WHEN PHARMACY_GROUP ='EPIC'THEN 'EPC'
    WHEN PHARMACY_GROUP ='KD'THEN 'KIN'
    WHEN PHARMACY_GROUP ='KROGER'THEN 'KRG'
    WHEN PHARMACY_GROUP ='WALMART'THEN 'WMT'
    WHEN PHARMACY_GROUP ='MEIJER'THEN 'MJR'
    WHEN PHARMACY_GROUP ='PUBLIX'THEN 'PBX'
    WHEN PHARMACY_GROUP ='GENOA'THEN 'GEN'
    WHEN pharmacy_group = 'HYVEE' THEN 'HYV'
    WHEN pharmacy_group = 'THRIFTY WHITE' THEN 'THF'
    -- WHEN pharmacy_group = 'PHARMACY FIRST' THEN 'TPS'
    WHEN pharmacy_group = 'AMAZON' THEN 'AMZ'
    ELSE pharmacy_group
  END AS chain_group,
  measurement,

  recon_metric AS cs_pharm_grte_type,
  target_rate AS cs_pharmacy_rate,
  target_pfee AS cs_target_pfee,

  recon_metric2 AS cs_pharm_grte_type2,
  target_rate2 AS cs_pharmacy_rate2,
  target_pfee2 AS cs_target_pfee2,

  recon_metric3 AS cs_pharm_grte_type3,
  target_rate3 AS cs_pharmacy_rate3,
  target_pfee3 AS cs_target_pfee3,
  'G' AS BG_FLAG,
  
"""

bnchmk_nadac_wac_prcs = """SELECT
  customer_id AS client, 
  gpi, 
  nadac_benchmark AS bnchmk
"""
...
FROM {_project}.{_staging_dataset}.gms_goodrx_drug_price
WHERE LOAD_DT = (
    SELECT MAX(LOAD_DT) 
    FROM {_project}.{_staging_dataset}.gms_goodrx_drug_price
    )
...
CASE
    WHEN CHAIN_GROUP NOT IN ({_pharm_list}) THEN 'IND'
    ELSE CHAIN_GROUP
    END AS CHAIN_GROUP_TEMP
FROM `{_project}.{_landing_dataset}.{_table_id}`
WHERE CHAIN_GROUP <> 'IND' and CUSTOMER_ID IN ({_customer_id})
AND MEASUREMENT_CLEAN NOT IN ('LTC','HIF','ALF')
...
SELECT DISTINCT NETWORK, CHAIN_GROUP, BG_FLAG, Rate, IFNULL(OFFSET_LEVEL, a.MEASUREMENT_CLEAN) AS MEASUREMENT_CLEAN, PERIOD, ratio
FROM (
    SELECT DISTINCT NETWORK, CHAIN_GROUP, BG_FLAG, Rate, MEASUREMENT_CLEAN, ratio, PERIOD
    FROM (
        SELECT sub1.*, 
               SUM(AWP) OVER (PARTITION BY CHAIN_GROUP, BG_FLAG, MEASUREMENT_CLEAN, PERIOD) AS chain_meas_awp,
               (SUM(AWP) OVER (PARTITION BY CHAIN_GROUP, BG_FLAG, MEASUREMENT_CLEAN, Rate, PERIOD)) / 
               (SUM(AWP) OVER (PARTITION BY CHAIN_GROUP, BG_FLAG, MEASUREMENT_CLEAN, PERIOD)) AS ratio
        FROM (
            SELECT TRIM(network) AS NETWORK, 
                   TRIM(chain_group) AS CHAIN_GROUP,
                   CASE WHEN GNRCIND = 'TRUE' THEN 'G'
                        ELSE 'B' END AS BG_FLAG,
                   Pharmacy_RATE AS Rate, 
                   TRIM(Measurement_clean) AS MEASUREMENT_CLEAN, 
                   SUM(FULLAWP) AS AWP, 
                   DOF,
                   CASE WHEN EXTRACT(YEAR FROM DOF) = EXTRACT(YEAR FROM CURRENT_DATE()) 
                        THEN 'CURRENT' ELSE 'PRIOR' END AS PERIOD
            FROM  `{_project}.{_landing_dataset}.{_table_id}`
            WHERE CAST(CUSTOMER_ID AS STRING) IN ({_customer_id}) 
              AND Client_Name NOT LIKE '%Specialty%' 
              AND Client_Name NOT LIKE '%LTC%' 
              AND Client_Name NOT LIKE '%WRAP%' 
              AND Client_Name NOT LIKE '%ALF%' 
              AND Client_Name NOT LIKE '%HIF%'
              AND network NOT LIKE '%MFN%'
              AND network NOT LIKE '%COR%'
              AND network NOT LIKE '%PRO%'
              AND TRIM(chain_group) IN ({_pharm_list})
              AND dof > '{_data_start}' 
              AND Rate > 0.3
              AND TRIM(medd_clm_ind) = 'Y' 
              AND TRIM(Pharmacy_Guarantee) = 'Y' 
            GROUP BY CHAIN_GROUP, NETWORK, MEASUREMENT_CLEAN, GNRCIND, RATE, DOF
        ) sub1
    ) sub2 
    WHERE ratio >{_ratio}
) a
LEFT JOIN (SELECT * FROM pbm-mac-lp-prod-ai.pricing_management.CustomMeasurement WHERE Customer_Id = {_customer_id}) b
ON OFFSET_CHECK = True AND a.MEASUREMENT_CLEAN = b.Measurement_Clean
"""
...
(SELECT DISTINCT VCML_ID
 FROM {_project}.{_landing_dataset_vcml}.{_table_id_vcml}
 WHERE customer_id IN ({_customer_id}) 
     AND Rec_curr_ind = 'Y' 
...
SELECT
q.* EXCEPT(rn), m.BEG_M_PRICE, 'G' AS BG_FLAG
FROM (
  SELECT
  MAC_GPI_CD AS GPI,
  ...
  FROM
    ...
    -- Using regex to look for "MAC" followed by exactly uppercase letters or digits that are the length of the customer_id
    -- Followed by optional extra letters or digits , which are not captured
    WHERE REGEXP_EXTRACT(MAC_LIST_ID, r'MAC([A-Z0-9]{{{_customer_id_len}}})[A-Z0-9]*') IN ({_customer_id})
...
SELECT 
    CUSTOMER_ID,
    bg_offset as BG_OFFSET,
    r30_r90_offset as R30_R90_OFFSET,
    has_splty as HAS_SPLTY,
    BG_FLAG,
    CASE WHEN SPLTY_FLAG IS NOT NULL THEN CONCAT(SPLTY_FLAG,MEASUREMENT) ELSE MEASUREMENT END AS MEASUREMENT,
    BREAKOUT,
    LABEL AS LABEL
""" 
...
INNER JOIN
        (
          ...
          FROM anbc-prod.fdl_ent_cnfv_prod.gms_ger_opt_taxonomy_final
...
INNER JOIN
        (
            ...
            ELSE
                 ...
                 END
            END AS CHAIN_GROUP_TEMP
            FROM (SELECT * FROM anbc-prod.fdl_ent_cnfv_prod.gms_ger_opt_vw_pharmacy_chain_mapping , (SELECT * FROM anbc-prod.fdl_ent_cnfv_prod.gms_ger_opt_taxonomy_final WHERE Customer_ID in ({_customer_id}) )x) y
        ) AS A2
...
INNER JOIN anbc-prod.fdl_ent_cnfv_prod.gms_ger_opt_taxonomy_final AS A5
...
INNER JOIN (
                    ...
                    ELSE
                         ...
                         END
                    END AS CHAIN_GROUP_TEMP
                    FROM (SELECT * FROM anbc-prod.fdl_ent_cnfv_prod.gms_ger_opt_vw_pharmacy_chain_mapping , (SELECT * FROM anbc-prod.fdl_ent_cnfv_prod.gms_ger_opt_taxonomy_final WHERE Customer_ID in ({_customer_id}) )x) y ) B2
ON
...
WITH
  ...
  FROM (
    ...
    END
      AS DISP_FEE_UNIT
    FROM
      `{_project}.{_dataset}.{_table_id1}`
    WHERE
      CLAIM_DATE BETWEEN DATE_SUB({_last_data}, INTERVAL {_time_lag} MONTH)
      AND {_last_data}
      AND BG_FLAG = 'G'
      AND (MEASUREMENT NOT LIKE '%S%' OR UPPER(BREAKOUT_OLD) NOT LIKE '%SPECIALTY%')
      AND CUSTOMER_ID IN (
      SELECT
        DISTINCT customer_id
      FROM
        `{_project}.{_dataset}.{_table_id2}`
      WHERE
...
FROM (
  ...
  FROM (
    ...
    END
      AS ZBD_COUNT
    FROM
       `{_project}.{_landing_dataset}.GER_OPT_CLIENT_PHARM_CLAIMS_STANDARD`
    WHERE customer_id IN ({_customer_id})
...
gpi_leakage_rankings_custom = """
SELECT
    *, 'G' AS BG_FLAG
FROM `{_project}.{_dataset}.{_table_id}`
WHERE customer_id in ({_customer_id})
AND leakage_rank <= {_leakage_rank}
"""

pstcosttype_check_custom = """
SELECT 
  CUSTOMER_ID, MEASUREMENT_CLEAN, PSTCOSTTYPE, count(distinct claim_id) as NUM_CLAIMS
  FROM `{_project_id}.{_dataset_id}.{_table_id}`
  WHERE customer_id in ({_customer_id})
  AND MEASUREMENT_CLEAN like 'R%'
  AND (PSTCOSTTYPE like 'MAC%' or PSTCOSTTYPE like 'TRU%')
  AND CLIENT_GUARANTEE like '%Y%'
  AND DOF BETWEEN '{_claim_start}' AND '{_claim_date}'
  group by 1, 2, 3
"""

gpi_drug_name_mapping = """
  SELECT
    GPI,
    DRUG_NAME
  FROM (
    SELECT
      ...
      gpi_nm AS DRUG_NAME,
      brnd_gnrc_cd,
      ROW_NUMBER() OVER (PARTITION BY GPI_CD ORDER BY gpi_nm DESC, brnd_gnrc_cd DESC) AS RN
    FROM
      `anbc-prod.fdl_ent_cnfv_prod.gms_v_drug_denorm_gcp`
    WHERE
      CURRENT_DATE('America/New_York') BETWEEN eff_dt AND exprn_dt
      AND gpi_cd NOT LIKE "% %"
      AND gpi_nm != " "
    GROUP BY
      1,2,3
...
gms_aetna_brand_specialty_surplus = """
SELECT CUSTOMER_ID AS CLIENT,
CUSTOMER_ID AS REGION,
MEASUREMENT,
BUCKET,
CHANNEL,
GUARANTEE_CATEGORY,
CAST(YTD_SURPLUS AS FLOAT64) AS YTD_SURPLUS,
CAST(RATE AS FLOAT64) AS RATE,
CAST(YTD_AWP AS FLOAT64) AS FULLAWP_ADJ,
CAST(YTD_SPEND AS FLOAT64) AS PRICE_REIMB
"""
...
SELECT
  ...
  MEASUREMENT,
  BG_FLAG,
  CAST(SUM(AWP) AS FLOAT64) AS RR_AWP,
  CAST(SUM(SPEND) AS FLOAT64) AS RR_SPEND
FROM
  `{_project_id}.{_dataset_id}.{_table_id}`
WHERE
  CUSTOMER_ID IN ({_customer_id})
  AND CLAIM_DATE BETWEEN DATE_SUB({_last_data}, INTERVAL 4 WEEK) and {_last_data}
GROUP BY
  CUSTOMER_ID,
  MEASUREMENT,
  BG_FLAG
...
SELECT
    ...
    CLAIMS_PROJ_WEEK,
    QTY_PROJ_WEEK,
    FULLAWP_ADJ_PROJ_WEEK,
    PHARM_CLAIMS_WEEK,
    PHARM_QTY_WEEK,
    PHARM_FULLAWP_ADJ_WEEK,
    PHARM_CLAIMS_PROJ_WEEK,
    PHARM_QTY_PROJ_WEEK,
    PHARM_FULLAWP_ADJ_PROJ_WEEK
"""
...
CAST(NDC AS STRING) AS NDC, 
BRND_GNRC_CD, 
CAST(NADAC AS FLOAT64) AS NADAC, 
CAST(WAC AS FLOAT64) AS WAC,
CAST(BNCHMK AS FLOAT64) AS BNCHMK,
"""

net_cost_guarantee_custom = """
select 
    CUSTOMER_ID as CLIENT,
    CAST(GPI AS STRING) AS GPI,
    TRIM(BRAND_GENERIC) as BG_FLAG,
    CASE WHEN REINVESTED_REBATES = 'Y' THEN CAST(guaranteed_truecost_net_price_year_1 AS FLOAT64)
        ELSE CAST(gross_price_year_1 AS FLOAT64) 
    END as NET_COST_GUARANTEE_UNIT   
    from `anbc-prod.fdl_gdp_ae_ent_enrv_prod.gms_truecost_gpi_14_drug_list` guarantees
inner join
(select distinct faf_id, customer_id, REINVESTED_REBATES from `anbc-prod.fdl_gdp_ae_ent_enrv_prod.gms_truecost_client_info` where CUSTOMER_ID IN ({_customer_id}) and '{_contract_eff_date}' between contract_start and contract_end) faf
on guarantees.faf_id = faf.faf_id
"""
...
SELECT 
  ...
  GPI14,
  'G' AS BG_FLAG,
  CAST(Cost_Fee_per_Unit AS FLOAT64) AS PHARM_AVG_ACC
FROM 
  `anbc-prod.fdl_ent_cnfv_prod.gms_gms_pgm_prm_68_cst_vantage_cost_file`
WHERE
  CURRENT_DATE() BETWEEN eff_dt AND thru_dt
QUALIFY ROW_NUMBER() OVER (PARTITION BY GPI14 ORDER BY EFF_DT DESC) = 1
"""

benchmark_nadac_price = """
SELECT
    GPI_CD AS GPI,
    NDC,
    'G' AS BG_FLAG,
    selected_unit_price AS BENCHMARK_CEILING_PRICE
"""

costsaver_avg_claim_qty = """
SELECT
  GPI, MEASUREMENT, ROUND(SUM(QTY)/SUM(CLAIMS_distinct),2) AS AVG_QTY_CLM
FROM
  `anbc-prod.fdl_gdp_ae_ds_pro_lp_share_ent_prod.combined_daily_totals`
WHERE
  CUSTOMER_ID IN (
  SELECT
    DISTINCT CUSTOMER_ID
  FROM
    `anbc-prod.fdl_gdp_ae_ent_enrv_prod.GER_OPT_TAXONOMY_FINAL`
  WHERE
    medd = 0 AND egwp = 0)
AND CLAIMS_distinct > 0
AND BG_FLAG = 'G'
AND (MEASUREMENT NOT LIKE '%S%' OR UPPER(BREAKOUT_OLD) NOT LIKE '%SPECIALTY%')
GROUP BY 1,2
"""
...
chain_subgroup_cmm = """
SELECT 
    DISTINCT CHAIN_SUBGROUP AS CHAIN_SUBGROUP_DISTINCT
"""
...
psot_schema = [
    ...
    bigquery.SchemaField("CLIENT_GUARANTEE_STRUCTURE", "STRING"),
]
