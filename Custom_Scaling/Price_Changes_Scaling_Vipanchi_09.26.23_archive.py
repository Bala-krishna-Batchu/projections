...
# CHANGE THIS
output_run_id = '_September_ScalingRuns' 

# CHANGE THIS
customer_id_list= [
"4146"
]
# customer_id_list = ["86C","92C","151EH"]
#['4515', '4499', '4539', '4558', '4594'] #  
# '4464', '4195', '4093'  are reported to Ryan
# 3148 - LP Custom scaling Steward health medicaid AZ
# 
bqclient = bigquery.Client()

def customer_id_finder(x):
    
    client_list = ["EH132","86C","92C","151EH","C29", "87C"]
    #CHANGE THIS everytime you have a client id that isnt FOUR characters long
...
## Ashim Check -- price change should usually be within 0.75 to 1.25 of the currentMAC price
dictionaries = [pcf_r30,pcf_r90,pcf_mail]

#CHANGE THIS -- comment out this section if scaling request specifically asks for increase or decrease beyond bounds
# Leave uncommented during most requests to avoid manual errors
for dictionary in dictionaries:
    for k,v in dictionary.items():
        if not (0.75 <= v <= 1.25):
            raise ValueError(f"Ashim-check -- Value out of bounds for key {k} : {v}")
...
for customer_id in customer_id_list:
    ...
    R30_R90_combo=[['MAC'+customer_id+'1','MAC'+customer_id+'3'],
                   ...
                   ['MAC'+customer_id+'1','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'4','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'5','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'6','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'7','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'8','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'4','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'8','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'11','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'14','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'15','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'16','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'18','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'33','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'41','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'44','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'47','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'55','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'59','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'66','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'69','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'77','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'87','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'97','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'177','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'199','MAC'+customer_id+'OK'],
                   ['MAC'+customer_id+'977','MAC'+customer_id+'OK'],
                  ]
    ...
    exclude_gpi=exclude_gpi.loc[(exclude_gpi.CLIENT==str(customer_id))|(exclude_gpi.CLIENT=='ALL')|(exclude_gpi.CLIENT=='nan')]
    ...
    print(price_changes[price_changes['GPI']=='57100010007505'][['MAC ID', 'PROPOSED MAC']], "post-specialty-GPI-exclusion")
    ...
    print(price_changes[price_changes['GPI']=='57100010007505'][['MAC ID', 'PROPOSED MAC']], "post-1026-price")
    print(price_changes.columns)
    
    
    ############## MAIL - RETAIL
    # CHANGE THIS to choose how to fix mail-retail
    # if customer_id in [ '4011','4464' , '4195', '4499', '4093', '4594' ]:
    if customer_id in ["3148", "4568", "4481", "4589"]:
    ...
    price_match_prices = price_match_prices.loc[(~price_match_prices['MAC ID'].isin([r90 for r30, r90 in R30_R90_combo])) & (price_match_prices['MAC ID']!='MAC'+customer_id+'2')]
...
#        price_match_prices = price_match_prices.loc[~price_match_prices['MAC ID'].isin(['MAC'+customer_id+'34', 'MAC'+customer_id+'91'])]
    ...
    if os.path.exists(output_folder + 'Independents_over_CVS_Violations.xlsx'):        
        ...
        with pd.ExcelWriter(output_folder + 'Independents_over_CVS_Violations.xlsx', engine="openpyxl",
            ...
            violations_retail_over_CVS.to_excel(writer, sheet_name=customer_id)
    print(price_changes[price_changes['GPI']=='57100010007505'][['MAC ID', 'PROPOSED MAC']], "post-First CVSSP check")
        ...
        #         violations_R90aboveR30.to_excel(writer, sheet_name=customer_id)
            ...
            if customer_id in ["4146"]:
                ...
                if len(b_prices) == 0 and len(price_changes.loc[price_changes['MAC ID']==a,:])>0:
                    if re.match('.*'+customer_id+'?([0-9]+)$', a).group(1) in ['5', '6', '7', '8'] and 'MAC'+customer_id+'9' in price_changes['MAC ID'].unique():
...
# # Redoing the check to ensure R30 > CVSSP
# # ############## CVS Indepedents (check designed to cap CVS price increases.) 
    
#     # Ensure parity compliance on r30
#     if customer_id in R90_exists:
#         price_changes.drop(columns=['PROPOSED MAC_MINRETAIL', 'PROPOSED MAC_MINRETAIL_90'], inplace=True)
#     else:
#         price_changes.drop(columns=['PROPOSED MAC_MINRETAIL'], inplace=True)
#     price_match_prices = price_changes[:]
#     price_match_prices = price_match_prices.loc[(~price_match_prices['MAC ID'].isin([r90 for r30, r90 in R30_R90_combo])) & (price_match_prices['MAC ID']!='MAC'+customer_id+'2')]
#     price_match_prices = price_match_prices.loc[~price_match_prices['MAC ID'].isin(['MAC'+customer_id+'41', 'MAC'+customer_id+'4'])]
#     price_match_prices = price_match_prices.groupby(['CUSTOMER_ID', 'GPI', 'NDC'], as_index=False)['PROPOSED MAC'].min().rename(columns={'PROPOSED MAC': 'PROPOSED MAC_MINRETAIL'})
...
#         # price_match_prices = price_match_prices.loc[~price_match_prices['MAC ID'].isin(['MAC'+customer_id+'34', 'MAC'+customer_id+'91'])]
#         price_match_prices = price_match_prices.groupby(['CUSTOMER_ID', 'GPI', 'NDC'], as_index=False)['PROPOSED MAC'].min().rename(columns={'PROPOSED MAC': 'PROPOSED MAC_MINRETAIL_90'})
#         print("price match prices post proposed mac minretail90 \n", price_match_prices[price_match_prices['GPI']=='57100010007505'])
#         price_changes = price_changes.merge(price_match_prices, how='left', on=['CUSTOMER_ID', 'GPI', 'NDC'])
#         if customer_id not in state_parity:
#             if 'MAC'+customer_id+'34' in price_changes['MAC ID'].unique():
#                 price_changes.loc[price_changes['MAC ID']!='MAC'+customer_id+'34', 'PROPOSED MAC_MINRETAIL_90'] = np.inf
...
#     # The below code was inside the "If condition"
#     violations_retail_over_CVS = price_changes.loc[
#         (price_changes['PROPOSED MAC']>price_changes['PROPOSED MAC_MINRETAIL'])].reset_index(drop=True)
#     price_changes['PROPOSED MAC'] = price_changes[['PROPOSED MAC', 'PROPOSED MAC_MINRETAIL']].min(axis=1) 
...
#     print(price_changes[price_changes['GPI']=='57100010007505'][['MAC ID', 'PROPOSED MAC']], "post- second CVSSP check")
    ...
    # CHANGE THIS
    price_changes['AT_RUN_ID']='CS2024012900000000000'+customer_id
...
# print(price_changes[price_changes['GPI']=='72600060000305'][['MAC ID', 'PROPOSED MAC']], "Final Print")
...
#CHANGE THIS  (the multiplying percentage.... here it is 1.25)....Clients with WTW and AON in their name
WTW_LIST_25_PERCENT_CAP = ["4951","4887","3266","5505","4195","C29","4093","4869","4158", "5072", "3217", "4164"]
price_changes_ok.loc[price_changes_ok.client_name.isin(WTW_LIST_25_PERCENT_CAP), "MACPRC"] = price_changes_ok.apply(lambda x: min(x["MACPRC"], x["CurrentMAC"] * 1.2499) if x["client_name"] in WTW_LIST_25_PERCENT_CAP else x["MACPRC"] , axis = 1  )

#CHANGE THIS -- Arbitrary fix to the super high increase in macprc .... adjust multiplier
price_changes_ok.loc[price_changes_ok.client_name.isin(customer_id_list), "MACPRC"] = price_changes_ok.apply(lambda x: min(x["MACPRC"], x["CurrentMAC"] * 2.5) if x["client_name"] in customer_id_list else x["MACPRC"] , axis = 1  )

#CHANGE THIS -- Arbitrary fix to the super high DECREASES in macprc .... adjust multiplier
price_changes_ok.loc[price_changes_ok.client_name.isin(customer_id_list), "MACPRC"] = price_changes_ok.apply(lambda x: max(x["MACPRC"], x["CurrentMAC"] * 0.75) if x["client_name"] in customer_id_list else x["MACPRC"] , axis = 1  )


# very specific changes only for GEL-313 --- comment out for normal running

# exclusion_df = bqclient.query(
#     f"""select *
# from `pbm-mac-lp-prod-ai.ds_production_lp.LP_Price_Recomendations_Custom`
# --CHANGE BOTH SUFFIX AND PREFIX BELOW
# WHERE AT_RUN_ID LIKE  'CS2023073100000000000172C'
#      """
# ).to_dataframe()
# mask = price_changes_ok.set_index(["MACLIST","GPI"]).index.isin(exclusion_df.set_index(["MACLIST","GPI"]).index)
# filtered = price_changes_ok[~mask]
# price_changes_ok = filtered.copy()

#CHANGE THIS  --- Check with Melanie... I added this code because without it we change the prices of specialty drugs
price_changes_ok = price_changes_ok[~price_changes_ok["MACLIST"].str.endswith("SX")]


# CHANGE THIS FILENAME
price_changes_ok[['MACLIST', 'GPI', 'GPPC', 'NDC11', 'NAME', 'EFFDATE', 'TERMDATE',
                    'CurrentMAC', 'client_name', 'MACPRC', 'AT_RUN_ID']].to_csv(output_folder + 'Price_Changes_Scaling_20231212.csv', index=False)


    ###############################################
    ##### CHANGE THIS
    ##### Hard coding price changes with a flat value - Ashim
    #### Code will change prices of all other GPIs too. To fix this, in the final BQ table, simply delete all other GPIs so that they don't get sent to RxClaim.
    
#     #use this one only when gpi is the filter for hadcoding  (else comment out)
#     list_prior_prices = price_changes_ok.loc[price_changes_ok["GPI"].isin(['66100052000330']), "MACPRC"] # saving old MACPRC so that we can put it into the new CurrentMAC 
#     price_changes_ok.loc[price_changes_ok["GPI"].isin(['66100052000330']), "MACPRC"] = 0.55 # enter hardcoded value here
#     price_changes_ok.loc[price_changes_ok["GPI"].isin(['66100052000330']), "CurrentMAC"] = list_prior_prices # setting old MACPRC to new CurrentMAC
    
    
    #use this one when both gpi and vcml are filters for hardcoding (else comment out)
# list_prior_prices = price_changes_ok.loc[((price_changes_ok["GPI"].isin(['66100052000330'])) &(price_changes_ok["MACLIST"].isin(['MAC41468']))), "MACPRC"] # saving old MACPRC so that we can put it into the new CurrentMAC 
# price_changes_ok.loc[((price_changes_ok["GPI"].isin(['66100052000330'])) & (price_changes_ok["MACLIST"].isin(['MAC41468']))), "MACPRC"] = 0.2857 # enter hardcoded value here
# price_changes_ok.loc[((price_changes_ok["GPI"].isin(['66100052000330'])) & (price_changes_ok["MACLIST"].isin(['MAC41468']))), "CurrentMAC"] = list_prior_prices # setting old MACPRC to new CurrentMAC
    
    ##############################################



#CHANGE THIS - FINAL CHANGE FOR HARDCODING -- ONLY EXPORT PRICES THAT MATCH YOUR MACLIST AND GPI (COMMENT OUT DURING NORMAL RUNS)
price_changes_ok = price_changes_ok[((price_changes["GPI"].isin(['66100052000330'])) & (price_changes["MACLIST"].isin(['MAC41468'])))]
######################################################

import datetime as dt
# CHANGE THIS TO True only when ready to upload
if False:
